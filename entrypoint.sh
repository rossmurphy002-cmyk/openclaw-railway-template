...
if [ -f "$CONFIG_FILE" ]; then
  node -e "
    ...
    delete cfg.gateway.controlUi.requireDevicePairing;
    delete cfg.gateway.controlUi.pairingRequired;
    if (cfg.gateway.auth) delete cfg.gateway.auth.controlUiPairingRequired;
    fs.writeFileSync('$CONFIG_FILE', JSON.stringify(cfg, null, 2));
    console.log('[entrypoint] Patched config');
  "
else
  echo '{"gateway":{"controlUi":{"dangerouslyAllowHostHeaderOriginFallback":true,"dangerouslyDisableDeviceAuth":true}}}' > "$CONFIG_FILE"
